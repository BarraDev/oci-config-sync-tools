...
- **Domain-based Profiles**: Uses OCI CLI profiles like `terraform-{domain}`
...
./oci-config-sync-tools/bin/tfvars-setup --help
```
...
./scripts/config-sync/bin/tfvars-setup terraform/config
```
...
### Generate tfvars and backend from vault

```bash
# Generate terraform.tfvars and _backend.tf for a specific directory
tfvars-setup terraform/config

# Specify domain explicitly
tfvars-setup --domain example.com terraform/infra

# Preview what would be generated
tfvars-setup --dry-run terraform/config

# Only include required variables (without defaults)
tfvars-setup --required-only terraform/config
```
...
tfvars-export terraform/config

# Create new vault item (fail if exists)
tfvars-export --create terraform/config

# Update existing item (fail if not exists)
tfvars-export --update terraform/config
```
...
tfvars-cleanup terraform/config

# Preview what would be removed
tfvars-cleanup --dry-run terraform/config
...
```hcl
# Generated by tfvars-setup on Sun Dec 21 2025
# Domain: example.com
...
terraform {
  backend "oci" {
    ...
    config_file_profile = "terraform-example.com"
  }
}
```
...
The generated `_backend.tf` uses a profile named `terraform-{domain}`.
...
oci-config-sync-tools/
├── bin/
│   ├── tfvars-setup      # Generate tfvars and backend from vault
│   ├── tfvars-export     # Export tfvars and backend to vault
│   └── tfvars-cleanup    # Remove generated files
├── lib/
...
# 2. On a new machine - generate from vault
tfvars-setup terraform/config
tfvars-setup terraform/infra
