...
provider_create_item() {
    ...
    item_json=$(jq -n \
        ...
        '{
            ...
            secureNote: { type: 0 },
            name: $name,
            notes: "Terraform configuration for OCI Kubernetes cluster. Generated by oci-config-export.",
            fields: $fields
        }')
...
provider_update_item() {
    ...
    item_json=$(echo "$existing_item" | jq \
        ...
        '.fields = $fields | .notes = "Terraform configuration for OCI Kubernetes cluster. Updated by oci-config-export."')
